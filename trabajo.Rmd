...
```{r}
df_completo.ts.train<-window(df_completo.ts,start=c(1,1),end=c(16,1))
df_completo.ts.valid<-window(df_completo.ts,start=c(16,2),end=c(17,3))


df_completo.autoarima<-auto.arima(df_completo.ts.train)

summary(df_completo.autoarima)
```
...
```{r}
bondad = function(real, media_real, aprox) {
  errores = real - aprox
...
bondad(df_completo.ts.valid, mean(df_completo.ts), df_completo.autoarima.pred.valid$mean)
```

Como podemos ver tanto el RMSE como el R\^2 son muy malos debido a que estamos intentando predecir a muchos trimestres vista. Por ello vamos ha crear un algoritmo que calcule a un mes vista y vaya reentrenando el modelo cada vez que le entra un dato nuevo. Esto tiene sentido porque, a pesar de estar asumiendo un coste computacional importante solo tenemos un dato por trimestre, por lo tanto nos lo podemos permitir. Pero, si que vamos a respetar el tipo de ARIMA sacado anteriormente (2, 2, 0) (0, 0, 1) [4]
...
for (y in ini_year:end_year){
  for (t in 1:4){
    if (!(y == 17 && t > 2)){
      p = arima_pred1(df_completo.ts, c(y, t))
      autoarima.pred_h1[nrow(autoarima.pred_h1)+1,] = as.data.frame(p)
      }
    }
}
autoarima.pred_h1
autoarima.pred_h1.ts = ts(autoarima.pred_h1$Point.Forecast, start = c(ini_year, 2), end = c(end_year, 3), frequency = 4)
```

```{r}
autoplot(cbind(autoarima.pred_h1.ts,df_completo.ts), xlab="Año", ylab="Indice precio vivienda")
```

Para comprobar bien el funcionamiento del modelo vamos a coger datos a coger como datos test el precio de la vivienda a partir de 2020 para ver como el modelo es capaz de predecir de manera bastante correcta el precio a un mes vista. Vamos a comprobar la bondad del modelo.

```{r}
bondad(df_completo.ts.valid, mean(df_completo.ts), autoarima.pred_h1.ts) 
```
...
```{r}
predicciones_test <- forecast(dept.ts.train.autoarima, h = length(dept.ts.valid))
...
```{r}
autoplot(cbind(predicciones_test$mean,dept.ts), xlab="Año", ylab="Indice precio vivienda")
```
